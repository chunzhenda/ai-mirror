...
srmodel_list_t *srmodel_mmap_init(const esp_partition_t *part)
{
    ...
    srmodel_list_t *models = static_srmodels;
    const void *root;
    esp_err_t err=esp_partition_mmap(part, 0, part->size, ESP_PARTITION_MMAP_DATA, &root, &models->mmap_handle);
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "Can not map %s partition!", part->label);
        return NULL;
    } else {
        ESP_LOGI(TAG, "partition %s size: %u by mmap", part->label, (unsigned)part->size);
    }
...
void srmodel_mmap_deinit(srmodel_list_t *models)
{
    if (models != NULL) {
        ...
        esp_partition_munmap(models->mmap_handle);
